...
int insertarListaOrdenado(Lista lista, void *dato) {
    ...
    Nodo no = crearNodo(lista ,dato);

if (lista->primero == NULL || strcmp(getNombre((ClientePtr)lista->primero->data), getNombre((ClientePtr)no->data))> 0){
        no->proximo = lista->primero;
        lista->primero = no;
...
Nodo ordenar=l->primero;
Nodo aux=NULL;
while(ordenar!=NULL){
        Nodo siguiente=ordenar->proximo;

    while(siguiente!=NULL){
        if(getEdad((ClientePtr)(ordenar->data))< getEdad((ClientePtr)siguiente->data)){

            aux=ordenar->data;
            ordenar->data=siguiente->data;
            siguiente->data=aux;
...
while (aux!=NULL){
   ...
   insertarFinal(listadupli,aux->data);
    aux=aux->proximo;
}
